...
/*
    ...
    vec3 base = pow(texture(material.ALBEDO, TexCoord).rgb, vec3(2.2));
    vec3 normal_n = getNormalFromMap();
   
    vec3 ambient = globalAmbient * base * texture(material.AMBIENT, TexCoord).r;

    vec3 lightDir;
    if (light.type == 0) // point light
        lightDir = normalize(light.position - FragPos);   // viewloc - ...
    ...
    float spec;
    if (blinnPhong != 0)
        spec = pow(max(dot(normal_n, halfway), 0.0), material.shininess*(texture(material.ROUGHNESS, TexCoord).r));
    else
